<template>
  <div class="card-types-page">
    <div class="page-header">
      <h2 class="page-title">卡类型管理</h2>
      <p class="page-desc">管理系统支持的卡类型，如年卡、月卡、次卡等</p>
    </div>
    ...
    <div class="table-container">
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="name" label="卡类型名称" width="150" />
    ...
    >
      ...
      >
        <el-form-item label="卡类型名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入卡类型名称" />
        </el-form-item>
...
const fetchData = async () => {
  ...
  try {
    ...
    const data = await cardTypeApi.getList(params)
...
const handleSubmit = async () => {
  await formRef.value?.validate(async (valid) => {
    if (valid) {
      try {
        if (isEdit.value) {
          await cardTypeApi.update(formData.id, formData)
          ElMessage.success('更新成功')
        } else {
          await cardTypeApi.create(formData)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      await cardTypeApi.delete(row.id)
