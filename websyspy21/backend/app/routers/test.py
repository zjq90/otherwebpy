...
router = APIRouter(prefix="/api/test", tags=["测试辅助功能"])
...
async def test_generate_data(db: AsyncSession = Depends(get_db)):
    try:
        print("正在生成安保人员数据...")
        personnel_list = [
        ...
        return {
            "message": "测试数据生成成功",
            "statistics": {
                "安保人员": len(personnel_list),
                "安保排班": len(schedules),
                "巡逻路线": len(routes),
                "监控设备": len(devices),
                "车辆出入记录": len(vehicles),
                "外来人员记录": len(visitors),
                "突发事件上报": len(emergencies),
                "保洁区域": len(cleaning_areas),
                "清洁记录": len(cleaning_records),
                "绿化植物": len(plants),
                "养护计划": len(maintenance_plans),
                "养护记录": len(maintenance_records)
            }
        }
        
    except Exception as e:
        await db.rollback()
        return {"message": f"生成测试数据失败: {str(e)}"}


@router.post("/clear-all-data", summary="清除所有数据")
async def test_clear_all_data(db: AsyncSession = Depends(get_db)):
    try:
        async with db.begin():
            await db.execute(text("DELETE FROM maintenance_record"))
            await db.execute(text("DELETE FROM maintenance_plan"))
            await db.execute(text("DELETE FROM green_plant"))
            await db.execute(text("DELETE FROM cleaning_record"))
            await db.execute(text("DELETE FROM cleaning_area"))
            await db.execute(text("DELETE FROM emergency_report"))
            await db.execute(text("DELETE FROM visitor_record"))
            await db.execute(text("DELETE FROM vehicle_record"))
            await db.execute(text("DELETE FROM monitor_device"))
            await db.execute(text("DELETE FROM patrol_record"))
            await db.execute(text("DELETE FROM patrol_route"))
            await db.execute(text("DELETE FROM security_schedule"))
            await db.execute(text("DELETE FROM security_personnel"))
        
        return {"message": "所有数据已清除"}
    except Exception as e:
        return {"message": f"清除数据失败: {str(e)}"}


@router.get("/statistics", summary="获取系统统计数据")
async def test_statistics(db: AsyncSession = Depends(get_db)):
    from sqlalchemy import select, func
    
    security_personnel_count = await db.scalar(select(func.count(SecurityPersonnel.id)))
    security_schedule_count = await db.scalar(select(func.count(SecuritySchedule.id)))
    patrol_route_count = await db.scalar(select(func.count(PatrolRoute.id)))
    patrol_record_count = await db.scalar(select(func.count(PatrolRecord.id)))
    monitor_device_count = await db.scalar(select(func.count(MonitorDevice.id)))
    vehicle_record_count = await db.scalar(select(func.count(VehicleRecord.id)))
    visitor_record_count = await db.scalar(select(func.count(VisitorRecord.id)))
    emergency_report_count = await db.scalar(select(func.count(EmergencyReport.id)))
    cleaning_area_count = await db.scalar(select(func.count(CleaningArea.id)))
    cleaning_record_count = await db.scalar(select(func.count(CleaningRecord.id)))
    green_plant_count = await db.scalar(select(func.count(GreenPlant.id)))
    maintenance_plan_count = await db.scalar(select(func.count(MaintenancePlan.id)))
    maintenance_record_count = await db.scalar(select(func.count(MaintenanceRecord.id)))
    
    pending_emergency = await db.scalar(
        select(func.count(EmergencyReport.id)).where(EmergencyReport.status != "已处理")
    )
    present_vehicles = await db.scalar(
        select(func.count(VehicleRecord.id)).where(VehicleRecord.status == "在场")
    )
    present_visitors = await db.scalar(
        select(func.count(VisitorRecord.id)).where(VisitorRecord.status == "在场")
    )
    ...
    return {
        ...
        "data": {
            "秩序维护管理": {
                "安保人员总数": security_personnel_count,
                "安保排班记录": security_schedule_count,
                "巡逻路线": patrol_route_count,
                "巡逻记录": patrol_record_count,
                "监控设备": monitor_device_count
            },
            "人员车辆管理": {
                "车辆出入记录": vehicle_record_count,
                "在场车辆": present_vehicles,
                "外来人员记录": visitor_record_count,
                "在场人员": present_visitors
            },
            "事件管理": {
                "突发事件上报": emergency_report_count,
                "待处理事件": pending_emergency
            },
            "环境保洁管理": {
                "保洁区域": cleaning_area_count,
                "清洁记录": cleaning_record_count
            },
            "绿化养护管理": {
                "绿化植物": green_plant_count,
                "养护计划": maintenance_plan_count,
                "养护记录": maintenance_record_count
            }
        }
    }
